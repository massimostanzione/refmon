...
struct dentry *singlefilefs_mount(struct file_system_type *fs_type, int flags, const char *dev_name, void *data) {
    ...
    if (unlikely(IS_ERR(ret)))
        printk("%s: error mounting onefilefs",REFMONFS_MODNAME);
    else
        printk("%s: singlefilefs is succesfully mounted on from device %s\n",REFMONFS_MODNAME,dev_name);
...
static int singlefilefs_init(void) {
    ...
    if (likely(ret == 0))
        ...
        printk("%s: failed to register singlefilefs - error %d", REFMONFS_MODNAME,ret);
...
static void singlefilefs_exit(void) {
    ...
    if (likely(ret == 0))
        ...
        printk("%s: failed to unregister singlefilefs driver - error %d", REFMONFS_MODNAME, ret);
}
...
MODULE_DESCRIPTION("REFMONFS - A mono-file file system for refmon");
